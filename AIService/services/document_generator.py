# AIService/services/document_generator.py
...
def auto_apply_suggestions(resume_json: Dict[str, Any], suggestions: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    ...
    logger.info(f"🔄 Applying {len(suggestions)} enhancement suggestions...")
    ...
    for i, suggestion in enumerate(suggestions, 1):
        ...
        logger.info(f"  {i}. [{suggestion_type.upper()}] {target_section}: {suggestion.get('reasoning', '')[:60]}...")
        ...
        try:
            if suggestion_type == "rephrase":
                if target_section == "summary":
                    ...
                    for proj in updated_resume.get("projects", []):
                        for j, bullet in enumerate(proj.get("bullets", [])):
                            if original_snippet.lower() in bullet.lower():
                                proj["bullets"][j] = suggested_text
                                applied_count += 1
                                break
                ...
                if target_section == "experience":
                    ...
                    if updated_resume.get("projects"):
                        if "bullets" not in updated_resume["projects"][0]:
                            updated_resume["projects"][0]["bullets"] = []
                        updated_resume["projects"][0]["bullets"].append(suggested_text)
                        applied_count += 1
            ...
            logger.warning(f"    ❌ Failed to apply suggestion {i}: {e}")
            continue
    
    logger.info(f"✅ Successfully applied {applied_count}/{len(suggestions)} suggestions")
...
def add_section(doc: Document, title: str, extra_space_before: bool = False):
...
def format_date_range(start: Dict[str, str], end: Dict[str, str], education_format: bool = False) -> str:
...
def generate_word_document(resume_data: Dict[str, Any]) -> bytes:
    """
    Generate a Word document from resume JSON data
    Returns the document as bytes for download
    """
    try:
        logger.info("📝 Generating Word document from resume data...")
        ...
        if resume_data.get("skills"):
            add_section(doc, "Skills")  # Remove extra_space_before to match other sections
